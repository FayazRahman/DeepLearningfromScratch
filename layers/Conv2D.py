...
class Conv2D:
    def __init__(
        self, num_filters, filter_shape, activation=None, strides=1, padding="same"
    ):
        ...
        if len(filter_shape) == 3:
            ...
            self.filters = np.random.randn(
                self.num_filters, self.f_h, self.f_w, self.f_n
            ) / (self.f_h * self.f_w)
        else:
            self.f_n = 1
            self.filters = np.random.randn(self.num_filters, self.f_h, self.f_w) / (
                self.f_h * self.f_w
            )
    ...
    def forward(self, inpt):
        ...
        assert n == self.f_n, "Expected kernel of depth {}, got {}".format(
            n, self.f_n
        )
        ...
        inpt = (
            np.pad(inpt, ((p, p), (p, p), (0, 0)), mode="constant")
            if inpt.ndim == 3
            else np.pad(inpt, p, mode="constant")
        )
    ...
    def backprop(self, dl_dout, alpha):
        ...
        for i, j, im_region in self.iterate_regions(dl_dinputs):
            for k in np.arange(self.num_filters):
                dl_dinputs[i : i + self.f_h, j : j + self.f_w] += (
                    dl_dout[i, j, k] * self.filters[k]
                )
